...
pub fn get_project(slug: String) -> Result<Value, String> {
    ...
    let content = fs::read_to_string(&file_path)
        .map_err(|error| format!("{}: {error}", display_path(&file_path)))?;
...
pub fn pick_project_image(source: String, slug: Option<String>) -> Result<Option<String>, String> {
    ...
    if !is_image_file(&selected_path) {
        return Err(
            "Selecciona un archivo de imagen valido: PNG, JPG, WEBP, GIF o SVG.".to_string(),
        );
    }
...
pub fn get_studio_content(kind: String, key: String) -> Result<Value, String> {
    ...
    match kind {
        ...
        "interests" => {
            let index = key
                .parse::<usize>()
                .map_err(|_| "Indice de interes no valido.".to_string())?;
            items
                .into_iter()
                .nth(index)
...
pub fn set_studio_content_hidden(
    ...
    if !matches!(kind, "development" | "certificates") {
        return Err(
            "La visibilidad solo esta disponible para En desarrollo y Certificados.".to_string(),
        );
    }

    let file_path = studio_content_file(&root_dir, kind)?;
    let mut items = read_json_array(&file_path)?;
    let id = slugify(&clean_required(
        &key,
        "El ID del contenido es obligatorio.",
    )?);
    let item = items
        .iter_mut()
...
pub fn save_certificate(
    ...
    let id = slugify(&clean_required(
        &input.id,
        "El ID del certificado es obligatorio.",
    )?);
...
#[tauri::command]
pub fn move_certificate(id: String, direction: String) -> Result<SavedContent, String> {
    let root_dir = portfolio_root()?;
    let file_path = studio_content_file(&root_dir, "certificates")?;
    let mut items = read_json_array(&file_path)?;
    let id = slugify(&clean_required(
    ...
    let current_index = items
        ...
        .ok_or_else(|| format!("No se encontro el certificado \"{id}\"."))?;
...
pub fn save_development_item(
    ...
    let id = slugify(&clean_required(
        &input.id,
        "El ID del elemento en desarrollo es obligatorio.",
    )?);
    ...
    if items.iter().any(|item| {
        ...
        return Err(format!(
            "Ya existe un elemento en desarrollo con el ID \"{id}\"."
        ));
    }
...
pub fn delete_development_item(id: String) -> Result<SavedContent, String> {
    ...
    if items.len() == original_len {
        return Err(format!(
            "No se encontro el elemento en desarrollo \"{id}\"."
        ));
    }
...
pub fn save_blog_post(
    ...
    let mut items = read_json_array(&file_path)?;
    let slug = slugify(&clean_required(
        &input.slug,
        "El slug de la nota es obligatorio.",
    )?);
...
pub fn update_project(
    ...
    if !old_file_path.exists() {
        return Err(format!(
            "No existe un proyecto con el slug \"{existing_slug}\"."
        ));
    }
    ...
    let mut all_projects = existing_projects
        .into_iter()
        .filter(|project| {
            project.get("slug").and_then(Value::as_str) != Some(existing_slug.as_str())
        })
        .collect::<Vec<_>>();
    all_projects.push(project);
    sort_projects(&mut all_projects);
...
pub fn move_project(slug: String, direction: String) -> Result<SavedContent, String> {
    ...
    let slug = slugify(&clean_required(
        &slug,
        "El slug del proyecto es obligatorio.",
    )?);
...
fn studio_content_item(
    ...
    let copy_en = item.pointer("/copy/en").unwrap_or(&Value::Null);
    let title = copy_es
        .get("title")
        .and_then(Value::as_str)
        .unwrap_or("Sin titulo")
    ...
    if kind == "blog" {
        if let Some(filter) = item
            ...
            .filter(|value| !value.is_empty())
        {
            metadata.insert("filter".to_string(), json!(filter));
        }

        if let Some(category) = copy_es
            .get("category")
            .and_then(Value::as_str)
            .map(str::trim)
            .filter(|value| !value.is_empty())
        ...
        if let Some(category_en) = copy_en
            .get("category")
            .and_then(Value::as_str)
            .map(str::trim)
            .filter(|value| !value.is_empty())
        {
            metadata.insert("categoryEn".to_string(), json!(category_en));
        }
    }
...
fn read_json_array(path: &Path) -> Result<Vec<Value>, String> {
    ...
    let content =
        fs::read_to_string(path).map_err(|error| format!("{}: {error}", display_path(path)))?;
...
fn build_certificate_value(input: &CertificateInput, id: &str) -> Result<Value, String> {
    let file_name = clean_required(
        &input.file_name,
        "El archivo del certificado es obligatorio.",
    )?;
    ...
    let certificate_type =
        clean_optional(Some(input.certificate_type.as_str())).unwrap_or(detected_type);
    let mime = clean_optional(Some(input.mime.as_str())).unwrap_or(detected_mime);
    let status =
        clean_optional(Some(input.status.as_str())).unwrap_or_else(|| "completed".to_string());
...
fn build_development_value(input: &DevelopmentInput, id: &str) -> Result<Value, String> {
    let title = clean_required(
        &input.title,
        "El titulo del elemento en desarrollo es obligatorio.",
    )?;
    let description = clean_required(
        &input.description,
        "La descripcion del elemento en desarrollo es obligatoria.",
    )?;
    let title_en = optional_or(&input.title_en, &title);
...
fn build_blog_value(input: &BlogInput, slug: &str) -> Result<Value, String> {
    ...
    if let Some(image_credit) = image_credit {
        if let Some(copy_es) = post.pointer_mut("/copy/es").and_then(Value::as_object_mut) {
            copy_es.insert("imageCredit".to_string(), json!(image_credit));
        }
    }

    if let Some(image_credit_en) = image_credit_en {
        if let Some(copy_en) = post.pointer_mut("/copy/en").and_then(Value::as_object_mut) {
            copy_en.insert("imageCredit".to_string(), json!(image_credit_en));
        }
    }

    if let Some(cover) = clean_optional(Some(input.cover.as_str())) {
        post.as_object_mut()
            .ok_or_else(|| "No se pudo construir la nota de blog.".to_string())?
            .insert("cover".to_string(), json!(cover));
    }

    if let Some(article_image) = clean_optional(Some(input.article_image.as_str())) {
        post.as_object_mut()
            .ok_or_else(|| "No se pudo construir la nota de blog.".to_string())?
            .insert("articleImage".to_string(), json!(article_image));
    }
...
fn build_interest_value(input: &InterestInput, title: &str) -> Result<Value, String> {
    ...
    let description = clean_required(
        &input.description,
        "La descripcion del interes es obligatoria.",
    )?;
...
fn remove_certificate_assets(root_dir: &Path, item: &Value) -> Result<(), String> {
    ...
    let Some(relative_path) = safe_relative_path(file_name) else {
        return Err(
            "El nombre del archivo del certificado no es una ruta local valida.".to_string(),
        );
    };
    ...
    for certificates_dir in [
        ...
        prune_empty_dirs_until(
            &certificates_dir,
            certificates_dir.join(&relative_path).parent(),
        )?;
    }
    ...
    match value {
        Value::String(text) => {
            if text.starts_with('/') && !text.starts_with("//") {
                let path = text.split(['?', '#']).next().unwrap_or("").trim();
...
fn sort_projects(projects: &mut [Value]) {
    projects.sort_by(|a, b| {
        let order_a = a.get("order").and_then(Value::as_i64).unwrap_or(i64::MAX);
        let order_b = b.get("order").and_then(Value::as_i64).unwrap_or(i64::MAX);
...
fn project_dynamic_modules(
    modules: &[ProjectModuleInput],
    flow: &[ProjectFlowStepInput],
) -> (Value, Vec<String>) {
